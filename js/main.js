...
const assignedListContainer = document.getElementById("assigned_list_container");
...
const nextId = () => todoList.length <= 0 ? 0 : Math.max(...todoList.map(task => task.id)) + 1;

const getTask = ({id, text, done}) => {
    return `
        <div class="list-item ${done ? "done" : "assigned"}" id="task_${id}">
        <span class="list-item__btn-container">
        <button class="list-item__btn ic_edit${done ? "_green" : ""}" title="Edit task" id="task_edit_${id}" onclick="editTask(event)"></button>
        </span>
        <input type="text" class="list-item__text" value="${text}" id="task_input_${id}" readonly />
        <span class="list-item__btn-container">
            <button class="list-item__btn ${done ? "ic_restore" : "ic_done_green"}" title="${done ? "Restore" : "Set done"}" id="task_makedone_${id}" onclick="changeTaskState(event)"></button>
...
const updateLists = () => {
    ...
    assignedHintText.style.display = assignedListContainer.innerHTML === "" ? "block" : "none";
    doneHintText.style.display = doneListContainer.innerHTML === "" ? "block" : "none";
};
